package joserodpt.mysticosskywars.plugin.gui;
...
public class GUIManager {
    ...
    public static void openSpectate(MSWPlayer p) {
        ...
        int i = 0;
        for (MSWPlayer s : p.getMatch().getPlayers()) {
            if (s.getPlayer() != null) {
                inventory.addItem(e -> {
    ...
    public static void openLanguage(MSWPlayer p) {
        ...
        for (MSWLanguage language : MysticosSkywarsAPI.getInstance().getLanguageManagerAPI().getLanguages()) {
            if (i == 17) {
                break;
            } else {
                inventory.addItem(e -> MysticosSkywarsAPI.getInstance().getPlayerManagerAPI().setLanguage(p, language), language.getIcon(), i);
            }
            ++i;
        }
    ...
    public static void openPluginMenu(MSWPlayer p, MysticosSkywarsAPI rsa) {
        GUIBuilder inventory = new GUIBuilder("<white><bold>Mysticos<aqua>Skywars <gray>v" + rsa.getVersion(), 27, p.getUUID());
        ...
        inventory.addItem(e -> {
            ...
            }.runTaskLater(MysticosSkywarsAPI.getInstance().getPlugin(), 1);
    ...
    public static void openPlayerProfile(MSWPlayer p) {
        ...
        if (!p.isInMatch()) {
            ...
            if (!MSWConfig.file().getBoolean("Config.Disable-Language-Selection")) {
                inventory.addItem(e -> {
                    p.closeInventory();
                    GUIManager.openLanguage(p);
                }, Itens.createItem(Material.BOOK, 1, TranslatableLine.MENU_LANG_TITLE.get(p), Collections.singletonList("&f> " + MysticosSkywarsAPI.getInstance().getLanguageManagerAPI().getLanguage(p.getLanguage()).getDisplayName())), 18);
            }

            if (!MSWConfig.file().getBoolean("Config.Disable-Player-Reset")) {
    ...
    public static void openAchievementGUI(MSWPlayer p) {
        ...
        inventory.addItem(e -> {
            p.closeInventory();
            AchievementViewerGUI v = new AchievementViewerGUI(p, MSWPlayer.PlayerStatistics.KILLS);
            v.openInventory(p);
        }, Itens.createItem(Material.DIAMOND_SWORD, 1, MSWPlayer.PlayerStatistics.KILLS.getDisplayName(p), Collections.singletonList(TranslatableLine.MENU_PLAYERP_VIEWITEM.get(p))), 10);

        inventory.addItem(e -> {
            p.closeInventory();
            AchievementViewerGUI v = new AchievementViewerGUI(p, MSWPlayer.PlayerStatistics.WINS_SOLO);
            v.openInventory(p);
        }, Itens.createItem(Material.LEATHER_BOOTS, 1, MSWPlayer.PlayerStatistics.WINS_SOLO.getDisplayName(p), Collections.singletonList(TranslatableLine.MENU_PLAYERP_VIEWITEM.get(p))), 12);

        inventory.addItem(e -> {
            p.closeInventory();
            AchievementViewerGUI v = new AchievementViewerGUI(p, MSWPlayer.PlayerStatistics.WINS_TEAMS);
            v.openInventory(p);
        }, Itens.createItem(Material.CHAINMAIL_CHESTPLATE, 1, MSWPlayer.PlayerStatistics.WINS_TEAMS.getDisplayName(p), Collections.singletonList(TranslatableLine.MENU_PLAYERP_VIEWITEM.get(p))), 14);

        inventory.addItem(e -> {
            p.closeInventory();
            AchievementViewerGUI v = new AchievementViewerGUI(p, MSWPlayer.PlayerStatistics.GAMES_PLAYED);
            v.openInventory(p);
        }, Itens.createItem(Material.FILLED_MAP, 1, MSWPlayer.PlayerStatistics.GAMES_PLAYED.getDisplayName(p), Collections.singletonList(TranslatableLine.MENU_PLAYERP_VIEWITEM.get(p))), 16);
    ...
    public static void openKitPreview(MSWPlayer p, MSWKit SWKit, int id) {
        if (!SWKit.hasItems()) {
            return;
        }
        ...
        inventory.setItem(event -> {
            p.closeInventory();
            if (id == 0) {
                PlayerItemsGUI pc = new PlayerItemsGUI(p, MSWBuyableItem.ItemCategory.KIT);
                pc.openInventory(p);
            } else {
                ShopGUI s = new ShopGUI(p, MSWBuyableItem.ItemCategory.KIT);
                s.openInventory(p);
            }
