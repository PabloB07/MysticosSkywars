...
public class PartiesManager extends PartiesManagerAPI {
    ...
    public PartiesManager(MysticosSkywarsAPI rs) {
        this.rs = rs;
    }

    @Override
    public void sendInvite(MSWPlayer emissor, MSWPlayer recetor) {
        if (emissor != recetor) {
            if (emissor.hasParty()) {
    ...
    @Override
    public void acceptInvite(MSWPlayer p) {
        MSWPlayer inviteOwner = this.getInvite(p);
    ...
    @Override
    public boolean checkForParties(MSWPlayer p, MSWMap swgm) {
