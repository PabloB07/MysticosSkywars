package joserodpt.mysticosskywars.api.database;
...
public class PlayerDataRow {
    ...
    public PlayerDataRow(OfflinePlayer p) {
        ...
        this.name = p.getName();
        this.coins = 0D;
        this.lang = MysticosSkywarsAPI.getInstance().getLanguageManagerAPI().getDefaultLanguage();
    ...
    public void setStatistics(MSWPlayer.PlayerStatistics pp, Boolean ranked, int value) {
        switch (pp) {
            case LOSES:
                if (ranked) {
