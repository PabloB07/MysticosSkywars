...
public class Text {
    ...
    public static String anonName() {
        List<String> nicks = MSWConfig.file().getStringList("Config.Random-Nicknames");
        return nicks.get(MysticosSkywarsAPI.getInstance().getRandom().nextInt(nicks.size())) + " #" + MysticosSkywarsAPI.getInstance().getRandom().nextInt(99);
    }

    public static String getDateAndTime() {
        DateFormat dateFormat = new SimpleDateFormat(MSWConfig.file().getString("Config.Time.Formatting"));
        Calendar cal = Calendar.getInstance();
        cal.setTime(new Date());
        cal.add(Calendar.HOUR_OF_DAY, MSWConfig.file().getInt("Config.Time.Offset"));
        cal.getTime();
    ...
    private static String randSp() {
        ...
        return sp.get(MysticosSkywarsAPI.getInstance().getRandom().nextInt(sp.size()));
    }
