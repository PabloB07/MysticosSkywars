...
public class ItemStackSpringer {
    ...
    public static ItemStack getItemDeSerialized(Map<String, Object> data) {
        ...
        ItemStack i = material.regionMatches(true, 0, "ITEMSADDER:", 0, 11)
                ? Itens.itemsAdder(material.substring(11), amount)
                : new ItemStack(Material.valueOf(material.toUpperCase()), amount);
            ...
            if (ItemCategories.ITEM_FLAGS.name().equals(key)) {
                ...
                for (String flagName : flags) {
                    try {
                        ...
                        MysticosSkywarsAPI.getInstance().getPlugin().getLogger().severe(flagName + " isn't a known ItemFlag (is this a bug?) Skipping this flag.");
                    }
                }
            }
    ...
    private static void debugPrint(Class<?> c, String s) {
        if (false)
            MysticosSkywarsAPI.getInstance().getPlugin().getLogger().warning(c.getName() + " -> " + s);
    }
    ...
    public static Enchantment getEnchantmentByName(String name) {
        switch (name.toLowerCase()) {
            ...
            default:
                MysticosSkywarsAPI.getInstance().getPlugin().getLogger().severe(name + " isn't a known Enchantment (is this a bug?) Skipping this enchant.");
                return null;
        }
    }
