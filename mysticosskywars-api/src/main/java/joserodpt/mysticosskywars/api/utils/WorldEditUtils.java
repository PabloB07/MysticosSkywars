...
public class WorldEditUtils {
    ...
    public static boolean schemFileExists(String name) {
        File folder = new File(MysticosSkywarsAPI.getInstance().getPlugin().getDataFolder(), "maps");
    ...
    public static void setBlocks(Location loc1, Location loc2, BlockType type) {
        ...
        try {
            ...
            MysticosSkywarsAPI.getInstance().getLogger().severe("Error while setting blocks!");
            MysticosSkywarsAPI.getInstance().getLogger().severe(e.getMessage());
        }
    }

    public static void pasteSchematic(String name, Location location, MSWMap map) {
                    ...
                    try (final EditSession editSession = WorldEdit.getInstance().newEditSession(BukkitAdapter.adapt(location.getWorld()))) {
                        ...
                        if (map != null) {
                            ...
                            for (Player p : location.getWorld().getPlayers()) {
                                ...
                                Text.send(p, "&fIf you want to change them, use WorldEdit to select the area and type /msw finish");
                            }
                        }
                    } catch (final WorldEditException e) {
                        MysticosSkywarsAPI.getInstance().getLogger().severe("Error while pasting schematic: " + name);
                        MysticosSkywarsAPI.getInstance().getLogger().severe(e.getMessage());
                    }
                }

            } catch (Exception e) {
                MysticosSkywarsAPI.getInstance().getLogger().severe("Error while pasting schematic!");
                MysticosSkywarsAPI.getInstance().getLogger().severe(e.getMessage());
            }
        }
    }
