...
public class MSWLanguage {

    private final String key;
    private final String displayName;
    private final ItemStack icon;

    private final File folder = new File(MysticosSkywarsAPI.getInstance().getPlugin().getDataFolder(), "languages");
    private final File translationFile;
    private final YamlConfiguration config;
    private JsonObject json = null;


    private final String version = MysticosSkywarsAPI.getInstance().getSimpleServerVersion();

    public MSWLanguage(File configFile) {
        ...
        if (folder.exists() && !translationFile.exists()) {
            try {
                ...
                Debugger.print(MSWLanguage.class, "Could not load language " + this.key + " - " + this.displayName + " -> Exception: " + e.getMessage());
            }
        }
    }
    ...
    public void downloadLanguageFile() throws Exception {
        ...
        try {
            ...
            MysticosSkywarsAPI.getInstance().getLogger().info("Downloaded minecraft language file for " + getKey() + " (" + version + ")!");
        } catch (IOException e) {
            // MysticosSkywarsAPI.getInstance().getLogger().severe("An error occurred while downloading the language file: " + e.getMessage());
            throw e;
        }
    }
