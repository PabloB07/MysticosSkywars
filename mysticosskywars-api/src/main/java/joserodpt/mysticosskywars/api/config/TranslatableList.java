...
public enum TranslatableList {
    ...
    LUCKYBLOCK_SPAWN_EVENT_TITLE(".Messages.Map.Events.LuckyBlock-Spawn"),
    LUCKYBLOCK_RAIN_EVENT_TITLE(".Messages.Map.Events.LuckyBlock-Rain"),
    LUCKYBLOCK_TREASURE_EVENT_TITLE(".Messages.Map.Events.LuckyBlock-Treasure"),
    ...
    public List<String> getInLanguage(String lang) {
        return Text.color(MysticosSkywarsAPI.getInstance().getLanguageManagerAPI().getLanguage(lang).getStringList(this.configPath));
    }

    public List<String> get(MSWPlayer player) {
        if (this == TITLE_ROOMJOIN) {
            List<String> list = getInLanguage(player.getLanguage());
            if (list.size() != 2) {
                MysticosSkywarsAPI.getInstance().getLogger().warning("Title RoomJoin must have 2 lines, but has " + list.size());
