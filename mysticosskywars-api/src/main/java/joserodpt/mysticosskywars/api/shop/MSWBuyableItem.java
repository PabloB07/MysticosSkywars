package joserodpt.mysticosskywars.api.shop;
...
public class MSWBuyableItem {
    ...
    public MSWBuyableItem(String configKey, String displayName, Material material, Double price, String permission, ItemCategory category, Map<String, Object> extras) {
    ...
    public MSWBuyableItem(String configKey, String displayName, Material material, Double price, String permission, ItemCategory category) {
        this(configKey, displayName, material, price, permission, category, new HashMap<>());
    }

    public MSWBuyableItem() {
    ...
    public void saveToConfig(Boolean save) {
        ...
        MSWShopsConfig.file().set(configPath + ".Displayname", getDisplayName());
        MSWShopsConfig.file().set(configPath + ".Material", getMaterial().name());
        MSWShopsConfig.file().set(configPath + ".Price", getPrice());
        if (permission != null || permission.isEmpty()) {
            MSWShopsConfig.file().set(configPath + ".Permission", getPermission());
        }
        if (!extras.isEmpty()) {
            MSWShopsConfig.file().set(configPath + ".Extras", getExtrasMap());
        }
        if (save)
            MSWShopsConfig.save();
    }
    ...
    public Double getPrice() {
        return this.getCategory() == MSWBuyableItem.ItemCategory.SPEC_SHOP ? this.price * this.amount : this.price;
    }
    ...
    public Pair<Boolean, String> isBought(MSWPlayer p) {
        if (this.getPrice() == 0) {
            return new Pair<>(true, "free");
        }
        return MysticosSkywarsAPI.getInstance().getDatabaseManagerAPI().didPlayerBoughtItem(p, this);
    }
    ...
    public enum ItemCategory {
        ...
        public String getCategoryTitle(MSWPlayer p) {
            switch (this) {
                case KIT:
